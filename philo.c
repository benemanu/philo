...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/08 15:51:07 by bhielsch          #+#    #+#             */
/*   Updated: 2023/02/08 15:56:33 by bhielsch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	take_fork(t_philo *philo)
{
	...
	if (is_philo_dead(philo->data) == 0)
	{
		lock_forks1(philo->data, philo->id);
		ind = 0;
	}
	if (is_philo_dead(philo->data) == 0)
		lock_forks2(philo->data, philo->id);
	else
	{
		if (philo->id % 2 == 0)
